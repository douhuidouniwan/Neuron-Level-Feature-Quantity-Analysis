...
class SigmaDataSet(Dataset):
    def __init__(self, root, model):
        ...
        if model == 'alexnet32':
            self.transformation = transforms.Compose([transforms.Resize((32, 32)), transforms.ToTensor(), normalize])
        else:
...
class AugmentDataSet(Dataset):
    def __init__(self, root, model):
        ...
        if model == 'alexnet32':
            self.transformation = transforms.Compose([transforms.Resize((32, 32)), transforms.RandomHorizontalFlip(), transforms.ToTensor(), normalize])
        else:
...
class ScanDataSet(Dataset):  #CenterCrop
    def __init__(self, root, model):
        self.root = root
        self.img_class = os.listdir(self.root)
        self.img_class.sort()
        ...
        if model == 'alexnet':
            self.transformation = transforms.Compose([transforms.Resize((227, 227)), transforms.ToTensor(), normalize])
        elif model == 'alexnet32' or model == 'AlexNet32':
            self.transformation = transforms.Compose([transforms.Resize((32, 32)), transforms.ToTensor(), normalize])
        else:
            self.transformation = transforms.Compose([transforms.ToTensor(), normalize])
        self.name_list = []
        self.label_list = []
        self.size = 0
        for i in range(len(self.img_class)):
...
class SimpleDataSet(Dataset):
    def __init__(self, root):
        ...
        for i in range(len(self.img_class)):
            ...
            for im in images:
                self.name_list.append(self.img_class[i] + '/' + im)
                self.label_list.append(str(i)+ '_' + im)
                self.size += 1

    def __getitem__(self, idx):
        label = self.label_list[idx]
        # image = self.transformation(Image.open(self.root+'/'+self.name_list[idx]))

        image = torch.load(self.root+'/'+self.name_list[idx])
