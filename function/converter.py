...
def conv_pre_mask(conv, mask, device='cuda'):
    ...
    masknum = conv.in_channels-in_channels
    print(f"conv2d in channels : {conv.in_channels} -> {in_channels}({masknum})")
...
def conv_post_mask(conv, mask, device='cuda'):
    ...
    masknum = conv.out_channels-out_channels
    print(f"conv2d out channels : {conv.out_channels} -> {out_channels}({masknum})")

    new_conv1 = torch.nn.Conv2d(in_channels=conv.in_channels,
                               out_channels=out_channels,
                               kernel_size=conv.kernel_size,
                               stride=conv.stride,
                               padding=conv.padding,
    ...
    new_conv1.weight.data = conv.weight.data[mask, :]
    # new_conv1.bias.data = conv.bias.data[mask]
